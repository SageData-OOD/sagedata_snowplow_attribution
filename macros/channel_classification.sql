...
{% macro default__channel_classification() %}
    ...
    case when lower(mkt_medium) in ('cpc', 'ppc') and regexp_count(lower(mkt_campaign), 'brand') > 0 then 'Paid_Search_Brand'
         ...
         when lower(mkt_medium) in ('email','newsletter') then 'Email'
         when lower(mkt_medium) in ('cpc', 'ppc', 'cpv', 'cpa', 'affiliates') then 'Other_Advertising'

        -- direct
        WHEN (refr_medium ILIKE '%unknown%' OR refr_medium IS NULL) AND refr_source IS NULL AND mkt_network IS NULL AND mkt_medium IS NULL
        THEN 'Direct'
          -- organic search
        WHEN refr_medium = 'search' AND mkt_medium IS NULL THEN 'Search'
        WHEN refr_medium = 'social' AND mkt_medium IS NULL THEN 'Social'
